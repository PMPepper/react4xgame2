...
import { AsyncTransport } from "./types";
...
type Methods = Record<string, AnyFunc>;
...
class AsyncConnection<RemoteMethods extends Methods, LocalMethods extends Methods | undefined = undefined> implements AsyncConnectionType<RemoteMethods> {
    call: AllPromised<Readonly<RemoteMethods>>;
    isReady: Promise<void>;
    ...
    replyHandlers: Record<number, ReplyHandler>;
    ...
    async init(localMethods?: LocalMethods) {
        ...
        this.isReady = new Promise<void>((resolve, reject) => {
            transport.onInit = (remoteMethodNamesArray) => {//TODO transport onInit should be a promise (also allows error handling)
